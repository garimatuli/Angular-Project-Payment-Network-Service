...
if ( $_SERVER['REQUEST_METHOD'] == 'GET')
	{
		...
		$query="SELECT ssn FROM electronic_address where ssn='$ssn'" ;
		...
		$query="SELECT id,identifier,ssn FROM electronic_address where ssn='$ssn' and type='P'" ;
		...
		if ($result = mysqli_query($conn, $query)) 
			   {
                 ...
                 {
                 array_push($data, array('id' => $row['id'] ,'ssn' => $row['ssn'],'identifier' => $row['identifier']));
                 }
                                      deliver_response(200,"User Details",$data);
